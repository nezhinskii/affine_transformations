...
class PrimitivesPainter extends CustomPainter {
  ...
  const PrimitivesPainter(
      {required this.primitives, required this.style, this.newPoints});
  ...
  void paint(Canvas canvas, Size size) {
    for (var primitive in primitives) {
      ...
      switch (primitive) {
        ...
        case Line():
          canvas.drawLine(
              primitive.startPosition, primitive.endPosition, paint);
        case Polygon():
          for (int i = 1; i < primitive.vertices.length; ++i) {
            canvas.drawLine(
                primitive.vertices[i - 1], primitive.vertices[i], paint);
          }
          canvas.drawLine(
              primitive.vertices.first, primitive.vertices.last, paint);
      }
    }
    if (newPoints != null) {
